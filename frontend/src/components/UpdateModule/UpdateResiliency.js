...
function ResilencyModal(props) {
    ...
    const loadResiliencyData = async ()=>{
        const  { data }  = await axios.get("http://localhost:8000/Resiliency_Details/" + `${ props.ResilencyID}`)
        console.log(data)
       setAllCriticalload(data.allcriticalload)
       setCriticalload(data.criticalload)
       setOutages(data.outages)
      setNoofoutages(data.noofoutages)
      setOutageduration(data.outageduration)
      setOutageTypes(data.outageType)
      setOthertype(data.othertype)
      setPowerfailure(data.powerfailure)
      setPowerfailureimpact(data.powerfailureimpact)
      }
      useEffect(()=>{
        loadResiliencyData();
    ...
    const UpdateData = async () => {  
      ...
      await axios({
        method:'post',
        url: "http://localhost:8000/Resiliency_Details/" + `${ props.ResilencyID}` + "/",
        data:FormField,
  ...
  <Form.Control type="text"  name='powerfailureimpact' value = {powerfailureimpact} onChange = {(e) => setPowerfailureimpact(e.target.value)} required/><br/>
        ...
        <Tippy content='Warning!: Your info is not saved if you press cancel all your info will be gone!'><button className='cancel' type='button' onClick={()=> {setMdShow(false);setRes(0)}}>Cancel</button></Tippy>
          ...
          <button className='save' type='submit' onClick={UpdateData}>Save</button>
        </div>
        </Form>
        </Modal.Body>
      </Modal>
