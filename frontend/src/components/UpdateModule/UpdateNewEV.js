...
function UpdateNewEV(props) {
    ...
    const loadNewEVdata = async ()=>{
        const  { data }  = await axios.get('http://127.0.0.1:8000/ExistingEV/' + `${ props.NewEvID }`)
        setNo_of_req_evchargers(data.no_of_req_evchargers)
      setCharger_type(data.charger_type)
      setCharge(data.EVcharge)
      setVisiting_vehicles(data.visiting_vehicles)
        
      }
      useEffect(()=>{
        loadNewEVdata();
      })
    const UpdateNewEVData = async (e) => {  
      e.preventDefault();
      setMdShow(false);
      ...
      await axios({
        method:'put',
        url: 'http://127.0.0.1:8000/NewEV/' + `${ props.NewEvID }`+ "/",
        data:FormField,
  ...
  <Tippy content='Warning!: Your info is not saved if you press cancel all your info will be gone!'><button className='cancel' type='button' onClick={()=> {setMdShow(false);setEV2(0)}}>Cancel</button></Tippy>
          ...
          <button className='save' type='submit' onClick={UpdateNewEVData}>Save</button>
        </div>
