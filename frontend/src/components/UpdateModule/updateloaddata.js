...
function Updateloaddata(props) {
  ...
  const loadloaddata = async ()=>{
    const  { data }  = await axios.get("http://localhost:8000/process_data" + `${ props.DataID }`+"/")
    console.log(data)
    setLoaddata(data.loaddata)
    setPDFbills(data.pdfbills)
  ...
  useEffect(()=>{
    ...
    const updateLoadData = async (e) => {  
      e.preventDefault();
      setMdShow(false);
      let FormField = new FormData()
      
      FormField.append('loaddata',loaddata)
      FormField.append('pdfbills', pdfbills)
      FormField.append('companyid', companyid)
      await axios({
        method:'put',
        url: "http://localhost:8000/process_data" + `${ props.DataID }` + "/",
        data:FormField,
    ...
    <div key={`inline-${type}`} className="mb-3">
      ...
      /> 
        ...
        <div className='buttons'>
          ...
          <button className='save' type='submit' >Save</button>
            ...
            >
              ...
              <Modal.Body>
            <Form className='PV-Form' action='#'  noValidate validated={validated} onSubmit={handleSubmit} encType="multipart/form-data">
       ...
       <Form.Text className="text-muted">
          ...
          <button className='save' type='submit' onClick={updateLoadData}  >Save</button>
        </div>
