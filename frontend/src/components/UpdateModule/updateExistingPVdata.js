...
function UpdateExistingPVdata(props) {
  ...
  const loadExistingPVdata = async ()=>{
    const  { data }  = await axios.get("http://localhost:8000/ExistingPV/" + `${ props.PvID }`)
    console.log(data)
    setExistingpvsize(data.existingpvsize)
    setExistingPVtype(data.existingpvtype)
    setsolarGenerationdata(data.solarGenerationdata)
    setSystemSLD(data.systemSLD)
  }
  useEffect(()=>{
    loadExistingPVdata();
  })
  const UpdateExistingPVData = async (e) => {  
    e.preventDefault();
    setMdShow(false);
    setPV(true);
    ...
    await axios({
      method:'put',
      url: 'http://127.0.0.1:8000/Existing_PV/' + `${ props.PvID }`,
      data:FormField,
  ...
  <Form.Group className="mb-3" controlId="formBasicEmail">
    ...
    <Form.Control type="text" placeholder="Solar PV Size(kW)" name="existingpvsize" defaultValue={existingpvsize} onChange={(e)=>setExistingpvsize(e.target.value)} required />
  ...
  <Form.Group controlId="formFile" className="mb-3">
       ...
       <Form.Label>Attach System SLD</Form.Label>
        ...
        <Tippy content='Warning!: Your info is not saved if you press cancel all your info will be gone!'><button className='cancel' type='button' onClick={()=>{setMdShow(false);setPV(0)}}>Cancel</button></Tippy>
          ...
          <button className='save' type='submit' onClick={UpdateExistingPVData}>Save</button>
        </div>
</Form>
        </Modal.Body>
      </Modal>
...
export default UpdateExistingPVdata
